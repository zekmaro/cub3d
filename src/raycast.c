...
/*   Updated: 2024/08/30 17:43:44 by iberegsz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void cast_ray(t_vars *vars, double ray_angle, int ray_id)
{
    ...
    while (y < draw_end)
    {        
        int tex_x = (int)(ray_x) % vars->unit_size;
        int tex_y = (int)((y - draw_start) * vars->unit_size / line_height);
        unsigned int color = get_texture_color(vars->textures[texture_index], tex_x, tex_y);
